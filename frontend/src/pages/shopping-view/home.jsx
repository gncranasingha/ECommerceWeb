  ...
  return (
    <div className='flex flex-col min-h-screen' >
      <div className='relative w-full h-[600px] overflow-hidden'  >
        ...
        onClick={() => setCurrentSlide(prevSlice => (prevSlice - 1 + slides.length) % slides.length)}
          className="absolute top-1/2 left-4 transform -translate-y-1/2 bg-white/80"
        >
          <ChevronLeftIcon className='w-4 h-4' />
        </Button>
        <Button variant='outline' size="icon"
          onClick={() => setCurrentSlide(prevSlice => (prevSlice + 1) % slides.length)}
          ...
          <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4' >
            {
              brandsWithIcon.map(branditems => 
                <Card onClick={()=>handleNavigateToListingPage(branditems, 'brand')} className="cursor-pointer hover:shadow-lg transition-shadow" >
                  <CardContent className="flex flex-col items-center justify-center p-6" >
      ...
      <section className='py-12' >
        <div className='container mx-auto px-4' >
          <h2 className='text-3xl font-bold mb-8' >Feature Products</h2>
